'use client';
     ...
     export default function Home() {
       ...
       const [visibleCategories, setVisibleCategories] = useState(categories.slice(0, 10));
       ...
       const filteredCategories = categories.filter(
         (category) =>
           ...
           category.tools.some((tool) => tool.title.toLowerCase().includes(searchQuery.toLowerCase())),
       );
       ...
       const favoriteTools = categories
         .flatMap((category) =>
           category.tools
             .filter((tool) => clientFavorites.includes(tool.title))
             .map((tool) => ({ ...tool, category: category.title, icon: category.icon })),
         );
       ...
       return (
         ...
         >
           ...
           {!isScrolled && <Hero onSearch={setSearchQuery} />}
               ...
               <>
                 ...
                 <div className="flex overflow-x-auto gap-6 pb-4 mb-2 scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200 dark:scrollbar-thumb-gray-600 dark:scrollbar-track-gray-800 sm:grid sm:grid-cols-2 sm:overflow-x-visible lg:grid-cols-5 sm:pb-0">
                   {favoriteTools.map((tool) => (
                     <div key={tool.title} className="flex-none min-w-[250px] sm:min-w-0">
                       <Card
                         icon={tool.icon}
                         title={tool.title}
                         desc={categories.find((c) => c.title === tool.category)?.desc || ''}
                         path={tool.path}
                         isTool
                         category={tool.category}
                         iconColor={categories.find((c) => c.title === tool.category)?.color}
                       />
                     </div>
                   ))}
                 </div>
